...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.result.upsert({
      where: { subjectId: validatedData.subject_id },
      update: {
        gradePoint: validatedData.grade_point,
    ...
    return NextResponse.json(
      {
        message: result ? 'Result updated successfully' : 'Result created successfully',
        resultId: result.id
      },
      { status: 201 }
    );
